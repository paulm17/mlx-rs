...
impl ChatTemplate {
    pub fn load(dir: &Path) -> Result<Self> {
        ...
        let bos_token = extract_token(&config, "bos_token").unwrap_or_else(|| "<|begin_of_text|>".to_string());
        let eos_token = extract_token(&config, "eos_token").unwrap_or_else(|| "<|end_of_text|>".to_string());

        let template = extract_template(&config)
            .unwrap_or_else(|| DEFAULT_LLAMA3_TEMPLATE.to_string());
...
mod tests {
    ...
    fn test_load_from_tokenizer_config() {
        ...
        let tmpl = ChatTemplate::load(dir.path()).unwrap();
